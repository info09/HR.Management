...
@Component({
  selector: 'app-employee-education-detail',
  templateUrl: './employee-education-detail.component.html',
})
export class EmployeeEducationDetailComponent implements OnInit, OnDestroy {
  ...
  loadEducation() {
    ...
    this.employeeService.getEducationByEducationIdByEducationId(this.config.data?.id).subscribe({
  ...
  saveChange() {
    ...
    if (this.utilityService.isEmpty(this.config.data.id)) {
      ...
      this.employeeService
        .updateEducationByEmployeeIdAndEducationIdAndInput(
